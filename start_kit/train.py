...
def eval_batch(data_loader, image_encoder, caption_encoder, ranker):
    ...
    rankings = []
    loss = []
    for i, (target_images, candidate_images, captions, lengths, meta_info) in enumerate(
        data_loader
    ):
        with torch.no_grad():
            ...
            rankings.append(
                ranker.compute_rank(candidate_ft + caption_ft, target_asins)
            )
            ...
            loss.append(
                triplet_avg(
                    anchor=(candidate_ft + caption_ft),
                    positive=target_ft,
...
def train(args):
    ...
    data_loader = get_loader(
        IMAGE_ROOT.format(args.data_set),
    ...
    ranker = Ranker(
        root=IMAGE_ROOT.format(args.data_set),
    ...
    save_folder = "{}/{}-{}".format(
        args.save, args.data_set, time.strftime("%Y%m%d-%H%M%S")
    )
    create_exp_dir(
        save_folder,
        scripts_to_save=[
    ...
    params = (
        image_encoder.get_trainable_parameters()
    ...
    for epoch in range(1):
        ...
        for i, (
            ...
            if i % args.log_step == 0:
                logging(
                    "| epoch {:3d} | step {:6d}/{:6d} | lr {:06.6f} | train loss {:8.3f}".format(
                        epoch, i, total_step, current_lr, loss.item()
                    )
                )
        ...
        if dev_score > best_score:
            ...
            torch.save(
                ...
                os.path.join(save_folder, "image-{}.th".format(args.embed_size)),
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--log_step", type=int, default=3, help="step size for printing log info"
    )
    parser.add_argument(
