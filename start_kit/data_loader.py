...
class Dataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        candidate_image = Image.open(
            os.path.join(self.root, candidate_img_name)
        ).convert("RGB")
        ...
        if self.return_target:
            ...
            target_image = Image.open(os.path.join(self.root, target_img_name)).convert(
                "RGB"
            )
            if self.transform is not None:
        ...
        tokens = (
            nltk.tokenize.word_tokenize(str(caption_texts[0]).lower())
            + ["<and>"]
