...
class Ranker:
    ...
    def update_emb(self, img_fts, batch_size, model):
        ...
        def append_emb(first, last):
            batch_ids = torch.tensor([j for j in range(first, last)], dtype=torch.long)
    ...
    def nearest_neighbors(self, inputs):
        ...
        for i in range(inputs.size(0)):
            [_, neighbor] = (self.data_emb - inputs[i]).pow(2).sum(dim=1).min(dim=0)

            neighbors.append(neighbor)
        return torch.tensor(neighbors).to(device=self.device, dtype=torch.long)
