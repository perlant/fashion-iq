...
def load_image_features(args):
    ...
    # train split
    fts_file = os.path.join(args.save, "b7_v2.{}.{}.th".format(args.data_set, "train"))
    ...
    # dev split
    fts_file_dev = os.path.join(
        args.save, "b7_v2.{}.{}.th".format(args.data_set, "val")
    )
    if os.path.isfile(fts_file_dev):
        ...
        fts_dev = utils.extract_features(
            data_loader_dev, attr_file_dev, args.attr2idx_file, device, args.image_model
        )
...
def eval_batch(fts, captioner, retriever, args, train_mode=False, optimizer=None):
        ...
        for d_turn in range(args.num_dialog_turns):
            ...
            with torch.no_grad():
                sentence_ids = captioner.get_caption(
                    target_img_ft, candidate_img_ft, target_attr, candidate_attr
                )
...
def train(args):
    ...
    retriever = RetrieverTransformer(
        captioner.get_vocab_size(),
        glove_emb,
        img_dim,
        args.hidden_unit_num,
        args.layer_num,
        args.attribute_num,
    ).to(device)
    ...
    for epoch in range(100):
        retriever.train()
        metrics = eval_batch(
            fts, captioner, retriever, args, train_mode=True, optimizer=optimizer
        )
        ...
        if epoch % args.checkpoint == 0:
            ...
            if dev_score > best_score:
                ...
                torch.save(
                    checkpoint_model, os.path.join(save_folder, "checkpoint_model.th")
                )
                cur_patient = 0
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--checkpoint", type=int, default=3, help="step size for saving models"
    )
    parser.add_argument(
        "--patient", type=int, default=2, help="patient for reducing learning rate"
    )
    ...
    parser.add_argument(
        "--beam_size", type=int, default=1, help="beam search branch size"
    )
